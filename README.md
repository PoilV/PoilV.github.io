...
- **图标**：优先加载站点自身的 `/favicon.ico`（惯例路径，实测约 2/3 站点可用），失败回退 [favicon.im](https://favicon.im)（解析站点 HTML 里声明的图标，svg/png 等格式都覆盖），再失败显示首字母回退块；图标永远最新，无需缓存。单条链接可加 `icon` 字段手动指定图标地址（最高优先级）。
