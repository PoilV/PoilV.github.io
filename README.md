...
- **图标**：优先加载站点自身的 `/favicon.ico`、`/favicon.svg`、`/favicon.png`（逐个尝试，失败自动级联），再回退 [favicon.im](https://favicon.im) 的 `https://favicon.im/{域名}?larger=true`，最后显示首字母回退块；图标永远最新，无需缓存。单条链接可加 `icon` 字段手动指定图标地址（最高优先级）。
