...
- **图标**：默认使用 [favicon.im](https://favicon.im) 的 `https://favicon.im/{域名}?larger=true`；个别站点图标不对时，给该条链接加 `icon` 字段手动指定（如 B 站那样）。有 `icon` 时两者并行加载、先完成的先显示，手动图标加载成功后自动替换，全部失败显示首字母回退块。
